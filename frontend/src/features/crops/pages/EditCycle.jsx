...
import { useUpdateCycleMutation } from "../cropApi";
import { clearSelectedCycle, selectSelectedCycle } from "../selectedCycleSlice";

const asDate = (iso) => (iso ? String(iso).slice(0, 10) : "");
const asString = (v) => (v === 0 || v ? String(v) : "");

/**
 * Map a decorated cycle (joined with the catalog crop + zone) onto
 * the cycle form's field shape.
 */
const toFormDefaults = (cycle) => ({
  name: cycle.name || "",
  cropId: cycle.cropId || "",
  zoneId: cycle.zoneId || "",
  status: cycle.status || "planned",
  growthStage: cycle.growthStage || "seedling",
  plantCount: asString(cycle.plantCount),
  dateSeeded: asDate(cycle.dateSeeded),
  expectedHarvest: asDate(cycle.expectedHarvest),
  actualHarvestDate: asDate(cycle.actualHarvestDate),
  notes: cycle.notes || "",
});
...
const EditCycle = () => {
  ...
  const cycle = useSelector(selectSelectedCycle);
  const [updateCycle, { isLoading: submitting }] = useUpdateCycleMutation();
  ...
  const t = getCropType(cycle.cropCategory);
  const TypeIcon = t.icon;

  const handleSubmit = async (values) => {
    try {
      const updated = await updateCycle({ id: cycle.id, ...values }).unwrap();
      dispatch(clearSelectedCycle());
      toast.success("Cycle updated", {
        description: `${updated.name ?? updated.cropName}'s details have been saved.`,
      });
      navigate("/app/crops", { replace: true });
    } catch (err) {
  ...
  return (
    <div className="flex flex-col lg:h-full lg:min-h-0">
      <Reveal duration={350}>
        <Link
          to="/app/crops"
          onClick={() => dispatch(clearSelectedCycle())}
            ...
            <div className="flex min-w-0 items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="truncate text-[11px] text-muted-foreground sm:text-xs">
                  Update {cycle.cropName ?? "this cycle"} on {cycle.zoneName}.
                </p>
              </div>
            </div>
          </div>
      ...
      <Reveal delay={140} duration={500}>
        <div className="glass-card texture-paper highlight-edge flex flex-col rounded-2xl p-4 sm:p-5 lg:min-h-0 lg:flex-1 lg:overflow-hidden">
          <CycleForm
            mode="edit"
            defaultValues={toFormDefaults(cycle)}
