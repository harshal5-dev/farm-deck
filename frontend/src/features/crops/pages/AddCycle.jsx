...
import { useCreateCycleMutation } from "../cropApi";
...
const AddCycle = () => {
  ...
  const handleSubmit = async (values) => {
    try {
      const created = await createCycle(values).unwrap();
      toast.success("Cycle planned", {
        description: `${created.name ?? created.cropName} is set up on ${created.zoneName}.`,
      });
      navigate("/app/crops", { replace: true });
    } catch (err) {
      toast.error("Could not plan cycle", {
        description: err?.data?.error?.message || "Please try again.",
      });
    }
  };

  const handleCancel = () => navigate("/app/crops");
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="min-w-0">
                ...
                <p className="text-[11px] text-muted-foreground sm:text-xs">
                  Pick a crop from the catalog, choose a field, set the seed
                  date — the cycle advances as it grows.
                </p>
              </div>
            </div>
