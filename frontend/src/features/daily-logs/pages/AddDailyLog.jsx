...
const AddDailyLog = () => {
  ...
  const { data: cyclesData, isLoading: cyclesLoading } =
    useListCyclesQuery();
  // Existing logs feed the form's duplicate-date guard
  // (mirrors the DB's `dl_uq_cycle_day` unique constraint).
  const { data: logsData } = useListLogsQuery(
    { cycleId },
    { skip: !cycleId }
  );
  ...
  const cycle = cycleBase
    ? {
        ...cycleBase,
        logType,
        logs: (logsData?.logs ?? []).map((l) => ({ ...l, cycleId })),
      }
    : null;
  ...
  const handleSubmit = async (values) => {
    try {
      const created = await createLog(values).unwrap();
      toast.success("Log added", {
        description: `${created.logDate} reading saved for the cycle.`,
      });
      navigate(`/app/crops/cycle/${cycleId}/logs`, { replace: true });
    } catch (err) {
      toast.error("Could not add log", {
