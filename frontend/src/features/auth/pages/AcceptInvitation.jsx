...
/**
 * AcceptInvitation — the screen an invitee lands on after clicking the
 * link in the team-invitation email.
 *
 * Flow:
 *   1. Read ?token=... from the URL. If missing → show themed "missing
 *      link" fallback.
 *   2. Call GET /auth/verify-invitation to load the invitee's details
 *      (name, email, role, tenant, expiry). While loading → themed
 *      FullPageLoader. On failure → themed InvitationInvalid screen
 *      with copy that matches the failure mode (expired, consumed,
 *      invalid, …).
 *   3. Render the themed AuthLayout + a glass card with the invitee
 *      header on top and the password form below. On submit → POST
 *      /auth/accept-invitation, then fetch profile, then dispatch
 *      setCredentials and navigate to /app (the auth cookies are set
 *      by the server so we never see the token in JS).
 */
...
/**
 * Map an RTK Query error to one of our InvitationInvalid reasons. We treat
 * 409 as "consumed" (already accepted) and everything else as "invalid" —
 * the verify endpoint doesn't return distinct status codes for expired vs
 * malformed tokens, but the backend service marks expired tokens in the
 * error so we get the right copy via the client-side expiry check.
 */
...
/**
 * InviteBrandPanel — left-half of the accept-invite screen. Mirrors the
 * AuthLayout panel (same gradient, same FarmScene art) but with copy
 * that fits the invitation flow: "Welcome to <tenant>".
 */
