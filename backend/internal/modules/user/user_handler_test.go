...
// Authorization for these routes is now enforced exclusively by
// middlewares.RequirePermission in the route registration — see
// internal/modules/user/http/routes.go. The Is*Allowed handler methods
// were removed and their tests deleted accordingly; permission logic is
// covered by the dedicated suite in internal/middlewares/auth_test.go
// (TestRequirePermission_*) and internal/domain/permissions_test.go
// (TestHasPermission, TestRolePermissionsAreMonotonic).
